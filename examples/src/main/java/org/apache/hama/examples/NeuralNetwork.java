...
public class NeuralNetwork {
    ...
    if (mode.equalsIgnoreCase("label")) {
      ...
      if (args.length >= 7) {
        try {
          ...
          System.err.println("LEARNING_RATE format invalid. It should be a positive double in range (0, 1.0)");
          return;
        }
      }
      if (args.length >= 8) {
        try {
          ...
          System.err.println("MOMEMTUM_WEIGHT format invalid. It should be a positive double in range (0, 1.0)");
          return;
        }
      }
      if (args.length >= 9) {
        try {
          ...
          System.err.println("REGULARIZATION_WEIGHT format invalid. It should be a positive double in range (0, 1.0)");
          return;
        }
      }
  ...
  private static void printUsage() {
    System.out.println("USAGE: <MODE> <INPUT_PATH> <OUTPUT_PATH> <MODEL_PATH>|<FEATURE_DIMENSION> <LABEL_DIMENSION> [<MAX_ITERATION> <LEARNING_RATE> <MOMEMTUM_WEIGHT> <REGULARIZATION_WEIGHT>]");
    System.out.println("\tMODE\t- train: train the model with given training data.");
    System.out.println("\t\t- evaluate: obtain the result by feeding the features to the neural network.");
    System.out.println("\tINPUT_PATH\tin 'train' mode, it is the path of the training data; in 'evaluate' mode, it is the path of the to be evaluated data that lacks the label.");
    System.out.println("\tOUTPUT_PATH\tin 'train' mode, it is where the trained model is stored; in 'evaluate' mode, it is where the labeled data is stored.");
    System.out.println("\n\tConditional Parameters:");
    System.out.println("\tMODEL_PATH\tonly required in 'evaluate' mode. It specifies where to load the trained neural network model.");
    System.out.println("\tMAX_ITERATION\tonly used in 'train' mode. It specifies how many iterations for the neural network to run. Default is 0.01.");
    System.out.println("\tLEARNING_RATE\tonly used to 'train' mode. It specifies the degree of aggregation for learning, usually in range (0, 1.0). Default is 0.1.");
    System.out.println("\tMOMEMTUM_WEIGHT\tonly used to 'train' mode. It specifies the weight of momemtum. Default is 0.");
    System.out.println("\tREGULARIZATION_WEIGHT\tonly required in 'train' model. It specifies the weight of reqularization.");
    System.out.println("\nExample:");
    ...
    System.out.println("Train a neural network by specify learning rate as 0.1, momemtum rate as 0.2, and regularization weight as 0.01:\n\tneuralnets.train hdfs://localhost:30002/training_data hdfs://localhost:30002/model 0.1 0.2 0.01");
